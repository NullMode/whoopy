...
from .utils import TokenInfo, RetryConfig
...
def run_async(coro):
    ...
    if loop is not None:
        ...
        raise RuntimeError(
            ...
            "Use WhoopClientV2 directly instead."
        )
...
class SyncCollectionMixin:
    ...
    def iterate(self,
        ...
        async def _iterate():
            items = []
            async for item in self._handler.iterate(
                start=start,
                end=end,
                limit_per_page=limit_per_page
            ):
...
class WhoopClientV2Sync:
    ...
    def from_config(cls,
                   ...
                   token_path: str = ".whoop_credentials.json") -> "WhoopClientV2Sync":
        """Create client from configuration files."""
        client = WhoopClientV2.from_config(
