...
from .base_v2 import BaseHandler, ResourceHandler, CollectionHandler, CombinedHandler
...
class RecoveryHandler(CollectionHandler[models.Recovery]):
    ...
    async def get_for_cycle(self, cycle_id: int) -> models.Recovery:
        ...
        try:
            ...
            if "404" in str(e):
                raise ResourceNotFoundError(
                    resource_type="Recovery for Cycle",
                    resource_id=str(cycle_id)
                )
            raise
...
class WorkoutHandler(CombinedHandler[models.WorkoutV2]):
    ...
    async def get_by_sport(self,
        ...
        """
        all_workouts = await self.get_all(
            start=start,
            end=end,
